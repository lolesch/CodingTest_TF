    ...
    public abstract class AbstractProvider<T> : MonoBehaviour where T : MonoBehaviour
    {
        ...
        private static bool InstanceExists(out T instance)
        {
            ...
            if (0 < candidates.Length)
            {
                ...
                Debug.Log($"Found {candidates.Length} existing {instance.name.ColoredComponent()}", candidates[0].gameObject);

                /// instance as component of "non-root" gameObjects
                if (instance.transform.parent != null)
                {
                    Debug.LogWarning($"{instance.name.Colored(Color.yellow)} is no root object and can't be moved by DonstDestroyOnLoad", _instance.Value.gameObject);
                    // concider force reparent the GameObject as root
                }

                for (var i = candidates.Length; i-- > 1;)
                {
                    DisableInstance(candidates[i]);
                }
        ...
        private static void DisableInstance(T candidate)
        {
            ...
            if (candidate != null)
            {
                Debug.LogWarning($"Disabled {_instance.Value.name.Colored(Color.red)} because there is already an Instance!", candidate);
